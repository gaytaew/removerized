...
- По скриншоту пользователя обнаружена отдельная причина: `OrtRun(): std::bad_alloc` — нехватка памяти WebAssembly при запуске ORMBG на 1024×1024. Это не серверная или сетевая ошибка. Версия 1.1.4 меняет default remover на динамический MODNet (25 MB) с проверенным размером 512×512; при `bad_alloc` у вручную выбранной тяжёлой модели редактор автоматически повторяет удаление фона на MODNet и показывает, что переключился.
- Исправление слито через PR `#8` в commit `84d9b848db683bbb35c1a40838e1b6491bafc4c6`; автодеплой run `31513301181` успешно прошёл за 1m19s. Активный каталог: `/opt/removerized/releases/84d9b848db683bbb35c1a40838e1b6491bafc4c6-31513301181`. Production-проверка версии 1.1.4 подтвердила MODNet в UI, готовый прозрачный результат и download action на тестовой картинке без ошибок ONNX. Removerized, Nginx, PostgreSQL и прежний сайт на IP остаются активными и возвращают HTTP 200.
